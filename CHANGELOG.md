...
- Use `window.setTimeout`/`window.clearTimeout` instead of bare globals for popout window compatibility
- Use `activeDocument` instead of `document` for popout window compatibility
- Apply sentence case to placeholder text in the quick-add form
- Replace the `builtin-modules` npm package with Node's native `node:module`
- Generate artifact attestations for `main.js` and `styles.css` in the release workflow so users can verify build provenance
...
[0.1.1]: https://github.com/zkbkb/obsidian-ClipBook/releases/tag/0.1.1
